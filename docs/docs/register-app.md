...
|                                             | autodiscovery.readwrite.all                  | Gets the scan job information of AvePoint Online Services.                                                                                                                                                                        |
...
1. **Set the Access Token URL**Choose the URL based on your organization's AvePoint environment:
...
3. Modify `Program.cs`. Open the `AosPublicApiTokenSample` folder and find the `Program.cs` file. Replace the file content with the following code:
   ...
   >
4. Execute the following command to run the project.`dotnet run`
