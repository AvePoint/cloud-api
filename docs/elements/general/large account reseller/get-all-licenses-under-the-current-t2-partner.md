# Retrieve All Pooled Subscriptions under the Current LAR

Use this API to retrieve all pooled subscriptions under the current LAR.
...
| API | Permission |
|-----------|--------|
| `/partner/external/v3/general/partners/t2-reseller/licenses/batch` | elements.t2.license.read.all |

## Request

This section outlines the details of the HTTP method and endpoint used to retrieve all pooled subscriptions under the current LAR.

| Method | Endpoint | Description |
|-----------|--------|------------|
| POST | `/partner/external/v3/general/partners/t2-reseller/licenses/batch` | Retrieves all pooled subscriptions under the current LAR. |
...
| metadata.totalCount | The total number of the pooled subscriptions under the current LAR. | integer |
...
| storage | The storage type: <ul><li>BYOS</li><li>AvePoint storage | string |
| subscriptionName | The display name of the pooled subscription. | string |
| source | The source of the pooled subscription: <ul><li>**5** - AvePoint pooled subscription</li><li>**6** - LAR organization pooled subscription</li></ul>| integer |
| unitPrice | The price charged per seat| string |
| paymentType | The payment type of the pooled subscription:<ul><li>**0** - Prepaid</li><li>**1** - Pay as you go</li></ul> | integer |
...
```json
https://graph.avepointonlineservices.com/partner/external/v3/general/partners/t2-reseller/licenses/batch?pageIndex=1&pageSize=50
```
...
```json
{
    "data": [
        {
            "licenseId": "9d32f11c-****-****-****-8fa729bc6103", // The ID of the pooled subscription
            "product": 2048, // The product code
            "poolName": "Cloud Backup for Microsoft 365", // The name of the pooled subscription
            "storage": "AvePoint Storage", // The storage type
            ...
            "allocatedUserSeat": "20", // The user seats allocated to resellers
            "availableUserSeat": "80", // The remaining unallocated user seats
    ...
    "metadata": {
        ...
        "totalCount": 1 // The total number of the pooled subscriptions under the current T2 partner
    }
}
